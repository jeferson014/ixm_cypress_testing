...
beforeEach("Login IXM", () => {
  cy.readFile("cypress/fixtures/csv/lots-dev-ori.csv", "utf8").then((data) => {
    cy.csvToJson(data).then((result) => {
      cy.writeFile("cypress/fixtures/lots-dev.json", result, "utf8");
    });
  });
...
describe("Lots", () => {
  it("Register Lots", () => {
    ...
    cy.addService(
      c.GET,
      "/MasterTableService/api/Client/SearchSupervisorByIdclient/59",
      "lstSupervisor"
    );
    ...
    cy.addService(
      c.GET,
      "/PurchaseSaleContractService/api/PurchaseSaleContract/59/0/1",
      "lstProduct"
    );
    cy.addService(
      c.GET,
      "/ContractService/api/ContractService/ListForLot/1/59/1/1",
      "lstContract"
    );
    ...
    cy.selectOption("Contract Service", va_Data.contract_service);
